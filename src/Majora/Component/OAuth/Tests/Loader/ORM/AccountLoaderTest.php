...
class AccountLoaderTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testRetrieveOnApplicationByUsernameQueryBuilder()
    {
        ...
        $queryBuilder = $accountLoader->retrieveOnApplicationByUsernameQueryBuilder($application->reveal(), Argument::any());

        $query = $queryBuilder->getQuery();

        $this->assertEquals($query->getDQL(), sprintf('SELECT ac FROM %s ac INNER JOIN ac.applications app WHERE ac.username = :username AND app.id = :application', AccountRepository::class));
    }
}