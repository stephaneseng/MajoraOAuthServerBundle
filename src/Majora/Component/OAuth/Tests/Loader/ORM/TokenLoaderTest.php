...
abstract class TokenLoaderTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testRetrieveByHashQueryBuilder()
    {
        ...
        $this->assertEquals($query->getDQL(), sprintf('SELECT t FROM %s t WHERE t.hash = :hash AND t.expireAt > :now', $this->tokenRepository->getClassName()));
    }
    ...
    public function testRetrieveExpiredQueryBuilder()
    {
        ...
        $this->assertEquals($query->getDQL(), sprintf('SELECT t FROM %s t WHERE t.expireAt < :date', $this->tokenRepository->getClassName()));
    }
}